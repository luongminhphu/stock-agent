...
def build_proactive_alert_embed(
    ...
    Args:
        event:   ProactiveWatchAlertFiredEvent carrying alert details.
        now_utc: Timestamp for embed footer; defaults to now.
    ...
    # Priority badge (only for THESIS_TRIGGER rules)
    if event.priority:
    ...
    # Label (thesis-trigger human-readable name)
    if event.label:
        embed.add_field(name="Label", value=event.label, inline=False)

    # Note / trigger description
    if event.note:
...
def build_proactive_batch_embed(
    ...
    Used when fired_count > 1 to avoid flooding the channel with N messages.
    Sends a single consolidated embed listing all alerts.

    Args:
        events:  List of ProactiveWatchAlertFiredEvent (at least 1).
        now_utc: Timestamp for footer.

    Returns:
        discord.Embed with one field per alert (max 10 shown).
    """
    now_utc = now_utc or datetime.datetime.now(tz=datetime.UTC)
    ...
    for ev in events[:10]:  # cap at 10 fields (Discord limit = 25)
    ...
    if len(events) > 10:
        embed.add_field(
            name="…",
            value=f"và {len(events) - 10} alert khác",
            inline=False,
        )

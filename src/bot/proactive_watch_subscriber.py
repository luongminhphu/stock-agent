...
_BATCH_WINDOW_SECONDS = 2  # accumulate events from same scan_event_id
...
class ProactiveWatchSubscriber:
    ...
    Args:
        client:     discord.Client (injected in bot on_ready via set_client())
        channel_id: Discord channel ID for alert delivery
    """

    def __init__(self, channel_id: int | None = None) -> None:
        self._channel_id = channel_id
        self._client: discord.Client | None = None
        # scan_event_id → (task, accumulated events)
        self._pending: dict[str, tuple[asyncio.Task, list[ProactiveWatchAlertFiredEvent]]] = {}

    def set_client(self, client: discord.Client) -> None:
        """Inject discord.Client after bot is ready (called in on_ready)."""
        self._client = client

    def set_channel_id(self, channel_id: int) -> None:
        """Override channel_id at runtime (called in on_ready after settings load)."""
        self._channel_id = channel_id

    def register(self) -> None:
        """Register on the event bus. Called once from bootstrap."""
        bus = get_event_bus()
    ...
    async def _handle(self, event: ProactiveWatchAlertFiredEvent) -> None:
        """Accumulate event; schedule a flush after the batch window."""
        scan_id = event.scan_event_id or event.event_id  # fallback: treat as solo
    ...
    async def _flush_after_window(
        ...
        """Wait for batch window, then send accumulated alerts to Discord."""

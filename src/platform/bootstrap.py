...
def _require(value: object, name: str) -> object:
    if value is None:
        raise RuntimeError(
            f"{name} is not initialised. Did you forget to call await bootstrap()?"
        )
    return value


def get_quote_service() -> object:
    return _require(_quote_service, "quote_service")


def get_ohlcv_service() -> object:
    return _require(_ohlcv_service, "ohlcv_service")


def get_ai_client() -> object:
    return _require(_ai_client, "ai_client")


def get_thesis_review_agent() -> object:
    return _require(_thesis_review_agent, "thesis_review_agent")


def get_thesis_suggest_agent() -> object:
    return _require(_thesis_suggest_agent, "thesis_suggest_agent")


def get_briefing_agent() -> object:
    return _require(_briefing_agent, "briefing_agent")


def get_why_agent() -> object:
    return _require(_why_agent, "why_agent")


def get_pretrade_agent() -> object:
    return _require(_pretrade_agent, "pretrade_agent")


def get_stress_test_agent() -> object:
    return _require(_stress_test_agent, "stress_test_agent")


def get_replay_agent() -> object:
    return _require(_replay_agent, "replay_agent")
...
def get_sector_rotation_agent() -> object:
    return _require(_sector_rotation_agent, "sector_rotation_agent")


def get_pnl_service_class() -> type:
    return _require(_pnl_service_class, "pnl_service_class")  # type: ignore[return-value]


def get_investor_profile_service() -> tuple:
    return _require(_investor_profile_service, "investor_profile_service")  # type: ignore[return-value]


def get_memory_consolidator() -> object:
    return _require(_memory_consolidator, "memory_consolidator")

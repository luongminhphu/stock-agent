...
Owner: market segment (Wave E1a — chuyển từ readmodel; writer là
market.TrendShiftDetector nên store phải ở market, readmodel chỉ query).
Responsibility: stores and retrieves the last known TechnicalSignalBundle per
symbol so TrendShiftDetector can compare current vs previous without
...
async def _persist_snapshot(session_factory, symbol: str, bundle_dict: dict) -> None:
    ...
    from src.market.models import TrendSnapshot
    ...
    await upsert_rows(
        ...
        log_event="trend_snapshot_store.persist_failed",
        symbol=symbol,
    )

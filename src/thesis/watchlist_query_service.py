"""WatchlistQueryService — read-model adapter for SignalEngineListener.
...
Pattern: session_factory injection (same as ThesisReviewListener,
ProactiveAlertAgent). One AsyncSession per call, closed on exit.

Never raises — returns [] on any error so SignalEngineListener degrades
gracefully.
"""
...
class WatchlistQueryService:
    """Read-only: fetch latest thesis health snapshots for SignalEngine."""

/**
 * quick-trade.js — B/S quick-trade buttons for Holdings table (Wave A: trade:confirmed)
 *
 ...
 *   - Always dispatches 'trade:confirmed' so app.js can refresh watchlist
 *     and AttentionPanel unconditionally (Wave A gap-wire).
 *
...
(function (global) {
  ...
  async function _handleConfirm() {
    ...
    try {
      ...
      // ─── Wave A gap-wire: notify watchlist + attention panel ─────────────
      // Dispatched unconditionally after every successful trade so app.js
      // can keep watchlist and AttentionPanel fresh without polling.
   ...
   * @param {{ fromThesisTab?: boolean }} opts
   *   fromThesisTab = true  → data-thesis-id luôn có (Thesis tab)
   *   fromThesisTab = false → data-thesis-id có thể có hoặc không (Trades tab)
   */
  function injectTradeButtons(tbody, opts) {
    ...
    tbody.querySelectorAll('tr[data-ticker]').forEach(function (row) {
      ...
      // Idempotent: skip rows that already have buttons
      if (targetCell.querySelector('.action-btns')) return;

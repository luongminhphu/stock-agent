...
export async function loadPortfolio() {
  ...
  try {
    ...
    // Wire QuickTrade — inject [B][S] buttons vào Trades tbody
    if (window.QuickTrade) {
      QuickTrade.init();
      const holdingsTbody = wrap.querySelector('[data-holdings-tbody]');
      if (holdingsTbody) QuickTrade.injectTradeButtons(holdingsTbody);

...
2. `failure_risks` phải liệt kê ÍT NHẤT 2 lý do cụ thể vì sao tín hiệu này có thể là false positive.
...
5. Trả về JSON hợp lệ, không có markdown hoặc giải thích ngoài JSON.
...
def build_user_prompt(ctx: SignalCredibilityContext) -> str:
    ...
    lines += [
        ...
        "",
        "Đánh giá độ tin cậy của tín hiệu này và trả về JSON theo schema đã định.",
    ]

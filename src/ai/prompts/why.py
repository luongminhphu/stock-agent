"""Prompt pack for WhyAgent — explain price movement.
Owner: ai segment.
"""

SYSTEM_PROMPT = """Bạn là chuyên gia phân tích kỹ thuật và cơ bản thị trường chứng khoán Việt Nam.
Nhiệm vụ: Giải thích nguyên nhân tăng/giảm đột biến của một mã cổ phiếu cụ thể.

Quy tắc:
- Luôn trả về JSON hợp lệ, không có text thừa bên ngoài JSON.
- Ngôn ngữ: tiếng Việt, ngắn gọn, actionable.
- Phân biệt rõ nguyên nhân kỹ thuật (breakout, volume, KLGD) vs cơ bản (tin tức, kết quả kinh doanh, ngành).
- Nếu thiếu dữ liệu, ghi rõ trong data_quality, KHÔNG bịa đặt nguyên nhân.
- confidence thấp khi thiếu tin tức xác nhận.
"""

def build_why_prompt(
    ticker: str,
    company_name: str,
    sector: str,
    change_pct: float,
    price: float,
    volume: int | None,
    ohlcv_summary: str,
    extra_context: str = "",
) -> str:
    direction = "TĂNG" if change_pct > 0 else "GIẢM"
    volume_text = f"{volume:,}" if volume else "không có dữ liệu"
    prompt = f"""Phân tích nguyên nhân biến động của {ticker} ({company_name}, ngành {sector}):

Biến động: {direction} {abs(change_pct):.2f}%
Giá hiện tại: {price:,.0f} đồng
Khối lượng giao dịch: {volume_text}

Dữ liệu OHLCV gần nhất:
{ohlcv_summary or "Không có dữ liệu lịch sử."}
"""
    if extra_context:
        prompt += f"\nThông tin bổ sung:\n{extra_context}\n"

    prompt += "\nGiải thích nguyên nhân theo JSON schema đã định nghĩa. Ưu tiên nguyên nhân có bằng chứng dữ liệu rõ ràng."
    return prompt

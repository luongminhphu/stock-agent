...
Owner: ai segment (ai.memory). Chuyển từ platform ở boundary fix B4: platform không
được import thesis/portfolio models; profile là aggregation phục vụ ContextBuilder.
Bảng ``investor_profiles`` giữ nguyên tên → không migration.
...
class InvestorProfileService:
    ...
    async def get_profile(self, user_id: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "ai.investor_profile.get_profile_failed",
                user_id=user_id,
                error=str(exc),
            )
            return {}
    ...
    async def build_snapshot(self, user_id: str) -> InvestorProfileSnapshot:
        ...
        logger.info(
            "ai.investor_profile.snapshot_built",
            user_id=user_id,
            active_thesis_count=active_thesis_count,
            win_rate_30d=win_rate,
            avg_hold_days=avg_hold,
    ...
    async def _count_active_theses(self, user_id: str) -> int:
        try:
            ...
            logger.warning("ai.investor_profile.thesis_count_failed", error=str(exc))
            return 0
    ...
    async def _extract_decision_insights(
        ...
        try:
            ...
            logger.warning("ai.investor_profile.decision_insights_failed", error=str(exc))
            return [], [], [], 0.0, 0.0
    ...
    async def _summarize_portfolio_bias(self, user_id: str) -> str:
        ...
        try:
            ...
            logger.warning("ai.investor_profile.portfolio_bias_failed", error=str(exc))
            return ""

...
Prompts: src/ai/prompts/suggest.py
"""
...
from src.ai.prompts.suggest import RESPONSE_SCHEMA, SYSTEM_PROMPT, build_user_prompt
...
def _pre_clean(text: str) -> str:
    """Clean AI artifacts BEFORE json.loads() to avoid parse errors.

    Handles:
    - Citation markers: [1], [2] — sonar-pro injects into strings
    - Trailing commas before } or ] — sonar occasionally emits
    - Literal newlines inside JSON string values — causes JSONDecodeError
    """
...
class ThesisSuggestAgent:
    ...
    async def suggest(self, ticker: str) -> ThesisSuggestionResult:
        ...
        messages = [
            ...
            {"role": "user", "content": build_user_prompt(ticker)},
        ]
        ...
        json_str = ""
        try:
            # Call directly — do NOT use `async with self._client`.
            # Client is a singleton managed by bootstrap/shutdown;
            # async with would call aclose() after the first request.

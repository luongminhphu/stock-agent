...
def _summary_row(**kwargs) -> ThesisSummaryRow:
    defaults = dict(
        id=1, ticker="HPG", title="Test", status="active",
        score=70.0, entry_price=50_000.0, target_price=65_000.0,
        stop_loss=45_000.0, upside_pct=30.0, risk_reward=3.0,
        current_price=None, pnl_pct=None,
        last_verdict="BULLISH", last_reviewed_at=None,
        created_at=_NOW,
        assumption_count=0, invalid_assumption_count=0,
        catalyst_count=0, triggered_catalyst_count=0,
    )
...
def test_dashboard_empty():
    d = DashboardResponse(
        user_id="u1", generated_at=_NOW,
        total_theses=0, active_count=0,
        invalidated_count=0, closed_count=0,
        avg_score=None, theses=[],
    )
...
def test_leaderboard_entries_ranked():
    entries = [
        LeaderboardEntry(
            rank=i + 1, thesis_id=i, ticker=f"T{i}",
            title=f"Thesis {i}", score=float(90 - i * 10),
            pnl_pct=float(20 - i * 5), last_verdict="BULLISH",
...
def test_timeline_events_ordered():
    events = [
        TimelineEvent(kind="created", ts=datetime(2025, 1, 1, tzinfo=timezone.utc),
                      summary="Thesis created", detail=None),
        TimelineEvent(kind="reviewed", ts=datetime(2025, 3, 1, tzinfo=timezone.utc),
                      summary="AI review: BULLISH", detail={"verdict": "BULLISH"}),
    ]
    tl = ThesisTimelineResponse(
        thesis_id=1, ticker="HPG", title="Test", events=events
    )
